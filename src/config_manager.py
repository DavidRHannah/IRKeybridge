"""
Configuration management module for the IR Remote Controller.

This module handles all configuration aspects including settings management,
remote profiles, and key mappings. It provides a comprehensive system for
...
class ActionType(Enum):
    """
    Enumeration of available action types for key mappings.

    Attributes:
        SINGLE: Single key press and hold
        COMBO: Key combination (multiple keys pressed simultaneously)
        SEQUENCE: Sequence of key presses with delays
        SPECIAL: Special controller actions (toggle modes, stop, etc.)
    """
...
class KeyMapping:
    """
    Represents a mapping between an IR code and a keyboard action.

    Attributes:
        action_type (ActionType): Type of action to perform
        keys (list[str] | str): Key(s) to execute
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        Convert KeyMapping to dictionary for JSON serialization.

        Returns:
            Dict[str, Any]: Dictionary representation of the mapping
        """
        return {
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "KeyMapping":
        """
        Create KeyMapping from dictionary data.

        Args:
            data (Dict[str, Any]): Dictionary containing mapping data

        Returns:
            KeyMapping: New KeyMapping instance
        """
        return cls(
...
class RemoteProfile:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "description": self.description,
            "mappings": {
                code: mapping.to_dict() for code, mapping in self.mappings.items()
            },
        }
...
class ConfigManager:
    ...
    def create_default_vizio_profile(self) -> RemoteProfile:
        ...
        mappings = {
            ...
            "0x1A": KeyMapping(
                ActionType.SPECIAL, "toggle_tap", "Toggle single tap mode"
            ),
